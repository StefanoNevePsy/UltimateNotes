...
| **Link tra note** | Inserisci una card-anteprima di un'altra nota sul canvas (menu + → "Collega nota"): mostra titolo e snippet, si collega con i connettori e si apre con un tocco — rete di note stile Obsidian. |
| **Lazo** | Disegna un lazo attorno a elementi e tratti per selezionarli insieme: spostali in blocco, raggruppali (i gruppi si muovono sempre insieme, anche senza cornice), separali o eliminali. |
| **Cornici** | Strumento "Cornice": disegna un riquadro per raggruppare visivamente una zona del canvas. Forme rettangolo/arrotondata/ellisse/sketchy, linee solid/tratteggiate/punteggiate, animazione, riempimento, etichetta. Spostando la cornice si sposta tutto il contenuto. |
| **Colori & font inline** | Nella stessa textbox puoi mescolare colori e font con i tag `{c:#FF0000}testo{/c}` e `{f:lora}testo{/f}` (applicati dalla barra formato sulla selezione). Il colore di default del testo si adatta automaticamente al tema. |
| **Palette colori** | 5 palette predefinite (Classica, Pastello, Neon, Terra, Mono) + palette personalizzate creabili dalle impostazioni; la palette attiva alimenta ruota rapida e barre stile. |
| **Incolla & sticker** | Incolla immagini dagli appunti (convertite in WebP, trasparenza preservata). Gli sticker della Samsung Keyboard / AI drawing assist arrivano via commitContent direttamente nei blocchi di testo. |
